...
void restartMCU() {
  char log_1[25];
  snprintf_P(log_1, sizeof(log_1), PSTR("Vou fazer reset em ESP.restart()\n"));Serial.print(log_1);
  //Serial.printf_P(PSTR("Vou fazer reset em ESP.restart()\n"));
...
{
  char log_1[25];
  snprintf_P(log_1, sizeof(log_1), PSTR("MEU: enterConfigMode\n"));Serial.print(log_1);
  //Serial.printf_P(PSTR("MEU: enterConfigMode\n"));
...
void enterConnectNet() {
  ...
  WiFi.mode(WIFI_STA); //Serial.println("Escolha de modo STA.");
  //Serial.printf("ONline getWiFiChannel(ssid): %i\n",getWiFiChannel(ssid));

  //Testa a ver se mando canal aos slaves quando o router mudou de canal:
  if(Online_mode){//So fazz isto se ja tivesse online (houve mudança de canal no router).
    //snprintf_P(log_1, sizeof(log_1), PSTR("Envio de canal aqui_1. \n"));Serial.print(log_1);
    //Serial.printf_P(PSTR("Envio de canal aqui_1\n"));
    //Serial.println("Envio de canal aqui_1");
    Canal_wifi = getWiFiChannel(ssid);
    //Serial.printf("ONline getWiFiChannel(ssid): %i\n",getWiFiChannel(ssid));

    if(Canal_wifi<1)Canal_wifi=1;
    if (Canal_wifi != Master_struct.Canal)for (int i = 0; i < 3; i++) {
        //Serial.printf("ONline share channel: %i\n",Canal_wifi);

        Enviar_canal_aos_slaves();
        delay(100);
      }
  }//else  Serial.printf("Estou no enterConnectNet mas offline e o getWiFiChannel(ssid) da %i.\n",getWiFiChannel(ssid));
  ...
  //WiFi.begin("Meuwifi_2", "passdowifi");
  ...
  if ((WiFi.status() != WL_CONNECTED)&&(cnt>=10)) {
    Tentativas_wifi++;
    snprintf_P(log_1, sizeof(log_1), PSTR("Tentei 10 segundos, por %d vezes.\n"),Tentativas_wifi);Serial.print(log_1);
    //Serial.printf_P(PSTR("Tentei 10 segundos, por %d vezes.\n"),Tentativas_wifi);
    //Serial.printf("Tentei 10 segundos, por %d vezes.\r\n",Tentativas_wifi);

    if(Tentativas_wifi >= 3){
      snprintf_P(log_1, sizeof(log_1), PSTR("Ja tentei 10 segundos, por %d vezes. Vou considerar offline.\n"),Tentativas_wifi);Serial.print(log_1);
      //Serial.printf_P(PSTR("Ja tentei 10 segundos, por %d vezes. Vou considerar offline.\n"),Tentativas_wifi);
      //Serial.printf("Ja tentei 10 segundos, por %d vezes. Vou considerar offline. \r\n",Tentativas_wifi);
      Tentativas_wifi=0;
      Online_mode=false;

      //Quando perco wifi n posso reajustar para canal 0.
      if(Master_struct.Canal>0)ajustar_canal_no_master(Master_struct.Canal);
      else{
        Master_struct.Canal=1;
        snprintf_P(log_1, sizeof(log_1), PSTR("Ajuste de canal aqui_1.\n"));Serial.print(log_1);

        //Serial.printf_P(PSTR("Ajuste de canal aqui_1.\n"));
        //Serial.println("Ajuste de canal aqui_1");
        ajustar_canal_no_master(Master_struct.Canal);
      }
      //
      //Testar a ver semaster qado fica nem wifi, se religa o esp no mesmo canal onde estava para n perder os slaves.

      millis_wifi_tentativa=millis();
      Tentar_wifi=false;
    }
  } else{
    snprintf_P(log_1, sizeof(log_1), PSTR("Connected to wifi on channel %i.\n"),WiFi.channel());Serial.print(log_1);
    //Serial.printf_P(PSTR("Connected to wifi on channel %i.\n"),WiFi.channel());
    //Serial.printf("Connected to wifi on channel %i.\n!",WiFi.channel());
    Online_mode=true;
    Tentar_wifi=false;
    //Nao falta aqui o reajustar do canal caso tenha mmudado?
  }
  //Ate aqui.
  /*
                                                                                                  //WiFi.begin(configStore.wifiSSID, configStore.wifiPass);//Original
                                                                                                  WiFi.begin(ssid, password);//MEU
...
void enterConnectCloud() {
  ...
  Blynk.config(auth, configStore.cloudHost, configStore.cloudPort);//Troquei o auth por auth_ .
  Blynk.connect(0);
