...
static bool config_load_blnkopt()
{
  ...
  BlynkParam::iterator auth = prov["auth"];//Troquei o auth por auth_ .
  BlynkParam::iterator host = prov["host"];
  BlynkParam::iterator port = prov["port"];

  if (!(ssid.isValid() && auth.isValid())) {//Troquei o auth por auth_ .
    return false;
  }
  ...
  if (auth.isValid()) { CopyString(auth.asStr(), configStore.cloudToken); }//Troquei o auth por auth_ .
